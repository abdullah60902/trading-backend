...
exports.getTeamMembers = exports.getReferralStats = exports.distributeCommissions = void 0;
 ...
 */
const distributeCommissions = async (userId, amount, currency, session) => {
    try {
        ...
        for (let i = 0; i < commissionRates.length; i++) {
            ...
            const uplineWallet = await Wallet_1.Wallet.findOne({ userId: uplineId, currency }).session(session);
            if (!uplineWallet)
                continue;
            // Credit the commission to the Earnings Wallet
            const currentEarnBal = Number(uplineWallet.earningsBalance.toString());
            uplineWallet.earningsBalance = mongoose_1.default.Types.Decimal128.fromString((currentEarnBal + commissionAmount).toFixed(8));
            ...
            const earningRecord = new Earning_1.Earning({
                ...
                type: earningType,
                currency,
                amount: mongoose_1.default.Types.Decimal128.fromString(commissionAmount.toFixed(8)),
                sourceId: user._id
            });
...
const getReferralStats = async (req, res) => {
    try {
        ...
        const directReferralsCount = await User_1.User.countDocuments({ referredBy: userId });
        // Total team size (all descendants in the path)
        const downlineUsers = await User_1.User.find({ referralPath: { $regex: `,${userId},` }, _id: { $ne: userId } }, { _id: 1 });
        const downlineIds = downlineUsers.map((u) => u._id);
        const totalTeamCount = downlineIds.length;
        const earningsRecords = await Earning_1.Earning.find({
            userId,
        ...
        const activeStakingCount = await StakingPlan_1.StakingPlan.distinct('userId', {
        ...
        const totalTeamSales = activePlans.reduce((sum, plan) => sum + Number(plan.lockedCapital.toString()), 0);
        res.status(200).json({
...
const getTeamMembers = async (req, res) => {
    try {
        ...
        const downline = await User_1.User.find({ referralPath: { $regex: `,${userId},` }, _id: { $ne: userId } }, { firstName: true, lastName: true, email: true, createdAt: true, referralPath: true, referredBy: true, status: true }).lean();
        ...
        const enrichedDownline = await Promise.all(downline.map(async (member) => {
            ...
            const userStaking = await StakingPlan_1.StakingPlan.find({ userId: member._id, status: 'active' });
            const individualSales = userStaking.reduce((sum, plan) => sum + Number(plan.lockedCapital.toString()), 0);
            return {
                id: member._id,
